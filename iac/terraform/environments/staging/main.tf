# SecureWatch360 — Staging environment
# Apply with: terraform -chdir=iac/terraform/environments/staging apply
...
locals {
  environment = "staging"
  site_url    = "https://staging.securewatch360.com"
}

module "supabase" {
  source          = "../../modules/supabase-project"
  organization_id = var.supabase_organization_id
  project_name    = "securewatch360-staging"
  region          = "us-east-1"
...
module "app" {
  ...
  app_env_vars = {
    ...
    APPROVAL_DEFAULT_SLA_HOURS    = "24"
    RISK_EXCEPTION_REVIEW_SLA_HOURS = "72"
    NODE_ENV                      = "production"
  }
}
