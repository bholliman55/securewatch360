...
export async function GET(request: Request) {
  try {
    ...
    const body = buildPolicyManifestJson(controls);

    const suffix = framework.length > 0 ? framework.toLowerCase() : "all";
    const filename = `securewatch360-policy-manifest-${suffix}.json`;
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to export policy manifest", message },
      { status: 500 }
    );
  }
}
