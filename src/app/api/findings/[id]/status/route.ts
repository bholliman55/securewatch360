...
import { autoProgressIncidentForResolvedFinding } from "@/lib/incidentAutoProgress";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    let incidentAutoProgressed = false;
    let incidentTransition: { incidentId: string | null; fromState: string | null; toState: string | null } | null =
      null;
    ...
    if (becameResolved) {
      ...
      try {
        const incidentResult = await autoProgressIncidentForResolvedFinding({
          tenantId: guard.tenantId,
          findingId: id,
          userId: guard.userId,
        });
        incidentAutoProgressed = incidentResult.progressed;
        incidentTransition = {
          incidentId: incidentResult.incidentId,
          fromState: incidentResult.fromState,
          toState: incidentResult.toState,
        };
      } catch {
        // Best-effort incident progression should not block finding status update.
      }
    }
