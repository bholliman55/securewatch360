...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error || !data) {
      throw new Error(error?.message ?? "Failed to approve risk exception");
    }
    ...
    await writeAuditLog({
      ...
      entityId: id,
      action: "risk.exception.approved",
      summary: "Risk exception approved",
      payload: {
        riskExceptionId: id,
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to approve risk exception", message },
      { status: 500 }
    );
  }
}
