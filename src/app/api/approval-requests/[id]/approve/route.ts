...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (existing.assigned_approver_user_id && existing.assigned_approver_user_id !== guard.userId) {
      return NextResponse.json(
        { ok: false, error: "Only the assigned approver can approve this request" },
        { status: 403 }
      );
    }
    ...
    if (existing.approval_type === "remediation_execution" && existing.remediation_action_id) {
      ...
      if (remediationError) {
        throw new Error(`Failed to apply approval to remediation action: ${remediationError.message}`);
      }
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to approve request", message },
      { status: 500 }
    );
  }
}
