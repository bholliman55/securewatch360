...
class ScanExecutionService {
  ...
  subscribeScanUpdates(scanId: string, callback: (scan: Record<string, unknown>) => void) {
    return supabase
      .channel(`scan:${scanId}`)
      .on(
        'postgres_changes',
  ...
  subscribeVulnerabilityUpdates(scanId: string, callback: (vulnerability: Record<string, unknown>) => void) {
    return supabase
