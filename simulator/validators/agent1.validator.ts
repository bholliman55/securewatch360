...
export function validateAgent1Response(ctx: AgentValidatorContext): AgentValidatorResult {
  ...
  const dbRecord =
    ctx.signals.auditRowsForRun.length > 0 || combined.includes(ctx.runId.toLowerCase());
  ...
  const checks: CheckItem[] = [
    {
      ...
      failureMessage:
        !inScope || triggered ? undefined : "No recon-related trigger signature in captured telemetry",
    ...
    },
    {
      ...
      failureMessage:
        !inScope || correctEvent ? undefined : "Expected external intel / exposure flavored synthetic events",
